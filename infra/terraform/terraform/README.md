...
Repo caveat: the embedded quickstart Stages use the reorganized path
`./src/app/quickstart/guestbook/env/<stage>`. The repo's `kargo/quickstart/stages.yaml`
still points at the pre-reorg `./src/app/env/<stage>` and would fail a promotion until
updated to match.
